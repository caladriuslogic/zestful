...
- `zestful notify` and `zestful watch` now also emit `agent.notified` events to the daemon's `/events` endpoint, alongside their existing `POST /notify` to the Mac app. Best-effort dual-emit — daemon outages don't break the notify path. First step in retiring the legacy `/notify` HTTP path; PR2 will re-home Mac-app side-effects (sound, APNs push, backend persist) onto the projection-driven path. **Transitional UX note:** during the PR1→PR2 window, `zestful watch <failing-cmd>` still triggers an urgent sound + APNs push via the legacy path (correct), but the projection-fed Mac-app overlay glyph will read the alert at Info severity (the `agent_notified` rule hardcodes `Severity::Info`). This visual/audible mismatch is not a bug — severity round-trip lands in PR2. See `docs/superpowers/specs/2026-04-29-cli-notify-events-design.md`.
- New event type `watch.completed` (replaces `agent.notified` for `zestful watch` exits). Carries structured `command` + `exit_code` + optional `duration_ms`. The new `watch_completed` notification rule decides severity from `exit_code` directly (Urgent on nonzero, Warn on zero) rather than parsing a synthesized message string.
- `Payload::AgentNotified` carries optional `severity_hint` and `push_hint` fields. Emitters (`zestful notify --severity` / `--no-push`) populate them; notification rules read them as input but remain authoritative for the per-firing severity and push decisions.

### Changed

- Notification rule severity and push policy are now per-firing on `NotificationBody` instead of per-rule constants. Visible consequence: `zestful watch <failing-cmd>` and the projection-fed Mac-app overlay now agree on severity (Urgent on failure) — closes the PR1→PR2 transitional UX asymmetry called out in the PR1 changelog. The notification projection's wire format gains a `push: bool` field on each `Notification` row.
